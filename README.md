...
- 총 실험 수: **22**
- 오늘 업로드 수: **3**
- 최고 LB: **0.5978689256**  (2026-04-26, probe experiment + fixed alpha 0.98 + fixed blend 0.2:0.8)
- 최신 기록: **2026-04-29** / structure experiment + small recent/change feature set added to stable baseline resulted in strong degradation to 0.606743338, confirming that the current task favors stable conservative setup over change-heavy features
<!-- AUTO:SUMMARY:END -->
...
| 날짜 | 실험 내용 | LB | 태그 | 커밋 |
|---|---|---:|---|---|
| 2026-04-29 | structure experiment + small recent/change feature set added to stable baseline resulted in strong degradation to 0.606743338, confirming that the current task favors stable conservative setup over change-heavy features | 0.5978689256 | exp | ac63041 |
| 2026-04-29 | structure experiment + target-wise alpha split 결과 0.598044066으로 소폭 악화, 공통 alpha 0.98 + blend 0.2:0.8이 더 안정적임을 확인 | 0.5978689256 | exp | 0a77c78 |
| 2026-04-29 | 5월 초까지 실험 중심으로 운영하기로 결정, 상위권 유지 및 top 60 목표 하에 핵심 실험 변화와 최고점 갱신 시 연구 로그를 함께 기록하는 방식으로 진행 | 0.5978689256 | exp | dac5b48 |
| 2026-04-27 | 논문 초안 작성 시작, 연구 방향과 핵심 메시지 정리 및 실험 결과 기반 본문 구조 설계 | 0.5978689256 | exp | 7a96776 |
| 2026-04-27 | alpha 0.98 + blend 0.15:0.85는 0.5979019616으로 소폭 악화, 현재 최고 구성은 stable baseline + alpha 0.98 + blend 0.2:0.8 with 0.5978689256로 확정하고 실험을 마감, 이후 논문 작성 단계로 전환 | 0.5978689256 | exp | 40d199d |
| 2026-04-26 | final probe result 확인, alpha 0.98 + blend 0.1:0.9는 0.5980970493로 소폭 악화, 현재 최고 구성은 alpha 0.98 + blend 0.2:0.8 with 0.5978689256로 확정 | 0.5978689256 | exp | a1e09be |
| 2026-04-26 | probe experiment + fixed alpha 0.98 + fixed blend 0.2:0.8 | 0.5978689256 | exp | eaeb50f |
| 2026-04-25 | stable baseline 기반 conservative shrink와 CatBoost-heavy ensemble 탐색, alpha 0.96 + blend 0.2:0.8에서 0.5979394488로 최고점 갱신, 실험 종료 후 논문 초안 작성 단계로 전환 | 0.5979394488 | exp | a457867 |
| 2026-04-25 | probe experiment + fixed alpha 0.96 + fixed blend 0.2:0.8 | 0.5979394488 | exp | 35307c0 |
| 2026-04-25 | stable baseline + fixed alpha 0.92 + fixed blend 0.3:0.7 | 0.5987808466 | exp | 2dc324b |
...
- 최근 실험:
  - 5월 초까지 실험 중심으로 운영하기로 결정, 상위권 유지 및 top 60 목표 하에 핵심 실험 변화와 최고점 갱신 시 연구 로그를 함께 기록하는 방식으로 진행
...
- 당일 최고 LB: **0.5978689256**
- 최근 실험:
  - probe experiment + fixed alpha 0.98 + fixed blend 0.2:0.8
  - final probe result 확인, alpha 0.98 + blend 0.1:0.9는 0.5980970493로 소폭 악화, 현재 최고 구성은 alpha 0.98 + blend 0.2:0.8 with 0.5978689256로 확정

### 2026-04-25
- 업로드 수: **3**
- 당일 최고 LB: **0.5979394488**
- 최근 실험:
  - stable baseline + fixed alpha 0.92 + fixed blend 0.3:0.7
  - probe experiment + fixed alpha 0.96 + fixed blend 0.2:0.8
  - stable baseline 기반 conservative shrink와 CatBoost-heavy ensemble 탐색, alpha 0.96 + blend 0.2:0.8에서 0.5979394488로 최고점 갱신, 실험 종료 후 논문 초안 작성 단계로 전환

### 2026-04-24
- 업로드 수: **3**
- 당일 최고 LB: **0.5991758604**
- 최근 실험:
  - stable baseline + fixed alpha 0.90
  - stable baseline에 shrink alpha 0.90과 fixed blend 0.4:0.6 적용, 0.6000012518로 크게 개선되며 11등 진입
  - stable baseline 유지, shrink alpha 0.90과 fixed blend 비율 조정 실험 진행, 0.3:0.7에서 0.5991758604로 최고점 갱신

### 2026-04-23
- 업로드 수: **3**
- 당일 최고 LB: **0.6064374291**
- 최근 실험:
  - stable baseline + fixed alpha 0.80
  - stable baseline + fixed alpha 0.78
  - stable baseline에서 shrink alpha 미세조정 실험 진행, alpha 0.78은 악화, 0.80은 개선, 0.85에서 0.6064374291로 최고점 갱신
<!-- AUTO:DAILY:END -->
...
- 다음 이어서 할 일:
  - `structure experiment + small recent/change feature set added to stable baseline resulted in strong degradation to 0.606743338, confirming that the current task favors stable conservative setup over change-heavy features` 기준으로 다음 실험 이어가기
