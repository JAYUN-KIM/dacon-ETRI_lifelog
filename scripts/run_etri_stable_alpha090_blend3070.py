...
LGB_W = 0.30
CAT_W = 0.70
OUT_PATH = BASE_DIR / "submissions" / "sub_stable_alpha090_blend3070.csv"
...
def prep_sensor_df(df, value_cols):
    ...
    keep = ["subject_id", "lifelog_date", "timestamp", "hour", "is_night", "is_day"] + [
...
def build_hr_features(sensor_map):
    ...
    hr_col = preferred[0] if len(preferred) > 0 else candidate_cols[0]
...
def main():
    print("=" * 90)
    print("LOAD TRAIN / SAMPLE SUBMISSION")
    print("=" * 90)
    ...
    core_personal_cols = [
        ...
        if c in [
            "m_activity_mean", "m_light_mean", "m_screen_use_mean",
            "heart_rate_mean", "heart_rate_std",
            "heart_rate_sleep_mean", "heart_rate_active_mean",
    ...
    print("valid split:", len(tr_idx), len(va_idx))
    ...
    for t in TARGETS:
        ...
        final_pred[t] = p
        print(
            f"[INFO] {t}: mean={final_pred[t].mean():.4f}, "
            f"std={final_pred[t].std():.4f}, "
            f"min={final_pred[t].min():.4f}, "
            f"max={final_pred[t].max():.4f}"
        )
