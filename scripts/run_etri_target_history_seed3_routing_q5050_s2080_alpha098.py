...
LGB_W = 0.20
CAT_W = 0.80
OUT_PATH = BASE_DIR / "submissions" / "sub_target_history_seed3_routing_q5050_s2080_alpha098.csv"
...
def build_target_history_features(train_feat, test_feat, target):
    key_cols = ["subject_id", "lifelog_date"]

    tr = train_feat[key_cols + [target]].copy()
    tr = tr.sort_values(["subject_id", "lifelog_date"]).copy()

    tr[f"{target}_hist_mean"] = (
        tr.groupby("subject_id")[target]
          .apply(lambda s: s.expanding().mean().shift(1))
          .reset_index(level=0, drop=True)
    )
    tr[f"{target}_hist_std"] = (
        tr.groupby("subject_id")[target]
    ...
    train_hist = tr[[f"{target}_hist_mean", f"{target}_hist_std", f"{target}_hist_cnt"]].reindex(train_feat.index)

    stats = train_feat.groupby("subject_id")[target].agg(["mean", "std", "count"]).reset_index()
    stats.columns = ["subject_id", f"{target}_hist_mean", f"{target}_hist_std", f"{target}_hist_cnt"]

    test_hist = test_feat[["subject_id"]].merge(stats, on="subject_id", how="left")
    test_hist = test_hist[[f"{target}_hist_mean", f"{target}_hist_std", f"{target}_hist_cnt"]]

    global_mean = float(train_feat[target].mean())
    global_std = float(train_feat[target].std()) if not pd.isna(train_feat[target].std()) else 0.0

    for h in [train_hist, test_hist]:
        h[f"{target}_hist_mean"] = h[f"{target}_hist_mean"].fillna(global_mean)
        h[f"{target}_hist_std"] = h[f"{target}_hist_std"].fillna(global_std)
        h[f"{target}_hist_cnt"] = h[f"{target}_hist_cnt"].fillna(0)
...
def main():
    ...
    print("TARGET HISTORY PRIOR EXPERIMENT: target-specific subject label history features are enabled")
    ...
    for seed in SEEDS:
        ...
        for t in TARGETS:
            ...
            hist_train, hist_test = build_target_history_features(train_feat, test_feat, t)
            X_train_t = pd.concat([X_train.reset_index(drop=True), hist_train], axis=1)
            X_test_t = pd.concat([X_test.reset_index(drop=True), hist_test], axis=1)
            model_features = final_features + list(hist_train.columns)

            med_t = X_train_t[model_features].median(numeric_only=True)
            X_train_t[model_features] = X_train_t[model_features].fillna(med_t)
            X_test_t[model_features] = X_test_t[model_features].fillna(med_t)
            ...
            lgb_model.fit(X_train_t[model_features], y)
            final_pred_lgb[t] += lgb_model.predict_proba(X_test_t[model_features])[:, 1] / len(SEEDS)
            ...
            cat_model.fit(X_train_t[model_features], y, verbose=False)
            final_pred_cat[t] += cat_model.predict_proba(X_test_t[model_features])[:, 1] / len(SEEDS)
