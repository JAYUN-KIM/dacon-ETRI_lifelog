...
LGB_W = 0.20
CAT_W = 0.80
OUT_PATH = BASE_DIR / "submissions" / "sub_usage_min_seed3_routing_q5050_s2080_alpha098.csv"
...
def build_usage_stats_features(sensor_map):
    ...
    for name, flag in [
        ("night", "is_usage_night"),
        ("evening", "is_usage_evening"),
        ("late", "is_usage_late"),
    ]:
        tmp = df[df[flag] == 1].groupby(key).size().reset_index(name=f"usage_{name}_event_count")
        out = out.merge(tmp, on=key, how="left")

    for c in ["usage_night_event_count", "usage_evening_event_count", "usage_late_event_count"]:
    ...
    print("[INFO] minimal usage features:", out.shape)
    print("[INFO] usage source:", p)
    print("[INFO] minimal usage columns:", [c for c in out.columns if c not in key])
    return out
