...
LGB_W = 0.20
CAT_W = 0.80
OUT_PATH = BASE_DIR / "submissions" / "sub_stable_changefeat_alpha098_blend2080.csv"
...
def add_change_features(df, cols):
    df = df.sort_values(["subject_id","lifelog_date"]).copy()
    for col in cols:
        grp = df.groupby("subject_id")[col]
        df[f"{col}_lag1"] = grp.shift(1)
        df[f"{col}_chg1"] = df[col] - df[f"{col}_lag1"]
        df[f"{col}_roll3_mean"] = grp.transform(lambda s: s.shift(1).rolling(3, min_periods=1).mean())
        df[f"{col}_dev_roll3"] = df[col] - df[f"{col}_roll3_mean"]
    return df
...
def main():
    ...
    change_cols = [c for c in ["m_screen_use_mean", "heart_rate_sleep_mean", "step_sum", "distance_sum"] if c in feat.columns]
    feat = add_change_features(feat, change_cols)
    ...
    for c in change_cols:
        change_features.extend([f"{c}_lag1", f"{c}_chg1", f"{c}_roll3_mean", f"{c}_dev_roll3"])
    change_features = [c for c in change_features if c in feat.columns]

    final_features = stable_candidates + sum([[f"{c}_prior_mean", f"{c}_prior_std", f"{c}_prior_cnt", f"{c}_dev"] for c in core_personal_cols], []) + change_features
    final_features = [c for c in final_features if c in feat.columns]

    print("n_change_features:", len(change_features))
    print("change_features:", change_features)
