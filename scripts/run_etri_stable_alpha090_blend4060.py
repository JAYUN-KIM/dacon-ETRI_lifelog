...
def main():
    ...
    pred_avg = LGB_WEIGHT * oof_lgb + CAT_WEIGHT * oof_cat
    ...
    log(f"time split FIXED_BLEND avg_logloss: {score_avg:.6f} | each: {each_avg}")
    ...
    log(f"fixed final blend: LGB_WEIGHT={LGB_WEIGHT:.2f}, CAT_WEIGHT={CAT_WEIGHT:.2f}")
    ...
    for t in TARGETS:
        p = LGB_WEIGHT * final_pred_lgb[t].values + CAT_WEIGHT * final_pred_cat[t].values
    ...
    save_path = OUT_DIR / "sub_stable_alpha090_blend4060.csv"
