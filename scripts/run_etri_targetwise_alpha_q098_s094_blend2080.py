...
LGB_W = 0.20
CAT_W = 0.80
OUT_PATH = BASE_DIR / "submissions" / "sub_targetwise_alpha_q098_s094_blend2080.csv"
...
def main():
    ...
    for t in TARGETS:
        ...
        alpha = ALPHA_Q if t in Q_TARGETS else ALPHA_S
        p = shrink_proba(p, alpha=alpha)
        p = clip_proba(p, 0.03, 0.97)
        final_pred[t] = p
        print(f"[INFO] {t}: alpha={alpha:.2f}, mean={final_pred[t].mean():.4f}, std={final_pred[t].std():.4f}, min={final_pred[t].min():.4f}, max={final_pred[t].max():.4f}")
