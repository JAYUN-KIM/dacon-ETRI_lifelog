...
LGB_W = 0.20
CAT_W = 0.80
OUT_PATH = BASE_DIR / "submissions" / "sub_usage_stats_seed3_routing_q5050_s2080_alpha098.csv"
...
def build_usage_stats_features(sensor_map):
    p = sensor_map.get("usage_stats")
    if p is None:
        print("[WARN] mUsageStats parquet not found")
        return None

    df = pd.read_parquet(p)
    if "subject_id" not in df.columns or "timestamp" not in df.columns:
        print("[WARN] mUsageStats missing subject_id/timestamp")
        return None
    ...
    df["lifelog_date"] = df["timestamp"].dt.floor("D")
    df["hour"] = df["timestamp"].dt.hour
    df["is_usage_night"] = ((df["hour"] >= 22) | (df["hour"] <= 6)).astype(int)
    df["is_usage_evening"] = ((df["hour"] >= 18) & (df["hour"] <= 23)).astype(int)
    df["is_usage_late"] = ((df["hour"] >= 0) & (df["hour"] <= 3)).astype(int)

    key = ["subject_id", "lifelog_date"]
    out = df.groupby(key).size().reset_index(name="usage_event_count")

    for name, flag in [("night", "is_usage_night"), ("evening", "is_usage_evening"), ("late", "is_usage_late")]:
        tmp = df[df[flag] == 1].groupby(key).size().reset_index(name=f"usage_{name}_event_count")
        out = out.merge(tmp, on=key, how="left")

    hour_feat = df.groupby(key)["hour"].agg(["mean", "std", "min", "max"]).reset_index()
    hour_feat.columns = key + ["usage_hour_mean", "usage_hour_std", "usage_hour_min", "usage_hour_max"]
    out = out.merge(hour_feat, on=key, how="left")

    cat_cols = []
    for c in df.columns:
        cl = c.lower()
        if any(k in cl for k in ["package", "app", "application", "name", "class"]):
            if c not in ["subject_id", "timestamp", "lifelog_date"]:
                cat_cols.append(c)

    for c in cat_cols[:3]:
        nunique = df.groupby(key)[c].nunique(dropna=True).reset_index(name=f"usage_{c}_nunique")
        out = out.merge(nunique, on=key, how="left")
        vc = df.dropna(subset=[c]).groupby(key + [c]).size().reset_index(name="cnt")
        if len(vc):
            top = vc.groupby(key)["cnt"].max().reset_index(name=f"usage_{c}_top_count")
            out = out.merge(top, on=key, how="left")

    numeric_cols = []
    for c in df.columns:
        if c in ["timestamp", "lifelog_date", "hour", "is_usage_night", "is_usage_evening", "is_usage_late"]:
            continue
    ...
    for c in numeric_cols:
        cl = c.lower()
        if any(k in cl for k in ["time", "duration", "total", "foreground", "count", "launch"]):
            preferred.append(c)
    selected_numeric = list(dict.fromkeys(preferred + numeric_cols))[:8]

    for c in selected_numeric:
        agg = df.groupby(key)[c].agg(["mean", "std", "min", "max", "sum"]).reset_index()
        agg.columns = key + [f"usage_{c}_mean", f"usage_{c}_std", f"usage_{c}_min", f"usage_{c}_max", f"usage_{c}_sum"]
        out = out.merge(agg, on=key, how="left")

        night = df[df["is_usage_night"] == 1].groupby(key)[c].agg(["mean", "sum"]).reset_index()
        night.columns = key + [f"usage_{c}_night_mean", f"usage_{c}_night_sum"]
        out = out.merge(night, on=key, how="left")

        evening = df[df["is_usage_evening"] == 1].groupby(key)[c].agg(["mean", "sum"]).reset_index()
        evening.columns = key + [f"usage_{c}_evening_mean", f"usage_{c}_evening_sum"]
        out = out.merge(evening, on=key, how="left")

    out["usage_night_event_ratio"] = out["usage_night_event_count"] / (out["usage_event_count"] + 1e-6)
    out["usage_evening_event_ratio"] = out["usage_evening_event_count"] / (out["usage_event_count"] + 1e-6)
    out["usage_late_event_ratio"] = out["usage_late_event_count"] / (out["usage_event_count"] + 1e-6)

    for c in out.columns:
        if c not in key:
            out[c] = pd.to_numeric(out[c], errors="coerce")

    print("[INFO] usage_stats features:", out.shape)
    print("[INFO] usage_stats source:", p)
    print("[INFO] usage cat cols:", cat_cols[:3])
    print("[INFO] usage numeric cols:", selected_numeric)
    return out
...
def main():
    ...
    sensor_map = {
        ...
        "usage_stats": get_sensor_path("mUsageStats"),
    }

    feat = base_df.copy()
    for ft in [build_activity_features(sensor_map), build_light_features(sensor_map), build_screen_features(sensor_map), build_charge_features(sensor_map), build_hr_features(sensor_map), build_pedo_features(sensor_map), build_usage_stats_features(sensor_map)]:
        if ft is not None:
    ...
    final_pred_lgb = pd.DataFrame(0.0, index=X_test.index, columns=TARGETS, dtype=float)
    final_pred_cat = pd.DataFrame(0.0, index=X_test.index, columns=TARGETS, dtype=float)
    ...
    for seed in SEEDS:
        ...
        for t in TARGETS:
            ...
            final_pred_lgb[t] += lgb_model.predict_proba(X_test[final_features])[:, 1] / len(SEEDS)
            ...
            final_pred_cat[t] += cat_model.predict_proba(X_test[final_features])[:, 1] / len(SEEDS)

    final_pred = pd.DataFrame(index=X_test.index, columns=TARGETS, dtype=float)
    for t in TARGETS:
        if t in Q_TARGETS:
            lgb_w, cat_w = 0.50, 0.50
        else:
        ...
        final_pred[t] = p
        print(f"[INFO] {t}: blend={lgb_w:.2f}:{cat_w:.2f}, mean={final_pred[t].mean():.4f}, std={final_pred[t].std():.4f}, min={final_pred[t].min():.4f}, max={final_pred[t].max():.4f}")
